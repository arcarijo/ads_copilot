...
export default function OnboardForm({ admin }: { admin: boolean }) {
  ...
  const title = admin ? "Client onboarding process" : "Ad account onboarding process";
  const entity = admin ? "client" : "ad account";
      ...
      {!clientId && (
        <div className="space-y-4 rounded-xl border border-[var(--line-subtle)] bg-[var(--surface-1)] p-6">
          ...
          <div>
            ...
            <p className={hintCls}>Website + socials are scraped once (max 5 pages) to build the AI strategy profile. Social links must be https and on a supported platform (Instagram, Facebook, TikTok, X, LinkedIn, YouTube).</p>
          </div>

          <h2 className="pt-2 font-medium text-[var(--ink-primary)]">2. Meta credentials</h2>
          <div className="grid grid-cols-2 gap-4">
            <div><label className={labelCls}>Ad Account ID *</label><input className={inputCls} value={form.metaAdAccountId} onChange={set("metaAdAccountId")} placeholder="e.g. 1234567890123456" /></div>
            <div><label className={labelCls}>Page ID *</label><input className={inputCls} value={form.metaPageId} onChange={set("metaPageId")} placeholder="e.g. 1098765432109876" /></div>
            <div><label className={labelCls}>System User name</label><input className={inputCls} value={form.metaSystemUserName} onChange={set("metaSystemUserName")} placeholder="e.g. MyBrand_AdBot" /></div>
            <div><label className={labelCls}>System User ID</label><input className={inputCls} value={form.metaSystemUserId} onChange={set("metaSystemUserId")} placeholder="e.g. 100000000000000" /></div>
            <div><label className={labelCls}>App ID</label><input className={inputCls} value={form.metaAppId} onChange={set("metaAppId")} placeholder="e.g. 000000000000000" /></div>
            <div />
          </div>
          <div>
        ...
        <div className="space-y-5 rounded-xl border border-[var(--line-subtle)] bg-[var(--surface-1)] p-6">
          ...
          <div className="border-t border-[var(--line-subtle)] pt-4">
            <button
              ...
              className="inline-flex items-center gap-1.5 rounded-lg bg-[var(--success)] px-5 py-2 text-sm font-semibold text-black shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:shadow-lg hover:brightness-105 active:translate-y-0 active:scale-[0.97]"
            >
              Done → view {entity} page
            </button>
          </div>
        </div>
      )}
